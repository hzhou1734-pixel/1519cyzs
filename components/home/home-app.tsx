...
import { PublishTypePage } from '@/components/pages/publish-type'
...
type Overlay =
  ...
  | { kind: 'search' }
  | { kind: 'publish-type' }
  | { kind: 'publish'; cat?: string; subCat?: string }
  | { kind: 'payment'; amount: number; label: string; onDone?: () => void; allowPoints?: boolean }
  | { kind: 'sub'; key: string }
...
export function HomeApp() {
  ...
  // 已付费解锁卖家手机号的闲置物品 id 集合
  const [unlockedIdles, setUnlockedIdles] = useState<Set<number>>(new Set())
        ...
        <div className="absolute inset-0 z-40 bg-background duration-300 animate-in slide-in-from-right">
          {top.kind === 'post' && (
            <PostDetail
              ...
              onBack={pop}
              showToast={showToast}
              phoneUnlocked={unlockedPosts.has(top.id)}
              onUnlockPhone={() => {
                if (!loggedIn) {
                  requestLogin()
                  return
                ...
                push({
                  ...
                  allowPoints: true,
                  onDone: () => {
                    setUnlockedPosts((prev) => new Set(prev).add(targetId))
                    pop()
                  },
                })
              }}
          ...
          {top.kind === 'idle' && (
            <IdleDetail
              ...
              onBack={pop}
              showToast={showToast}
              phoneUnlocked={unlockedIdles.has(top.id)}
              onUnlockPhone={() => {
                if (!loggedIn) {
                  requestLogin()
                  return
                ...
                push({
                  ...
                  allowPoints: true,
                  onDone: () => {
                    setUnlockedIdles((prev) => new Set(prev).add(targetId))
                    pop()
                  },
                })
              }}
          ...
          {top.kind === 'publish-type' && (
            <PublishTypePage
              onBack={pop}
              onSelect={(cat, subCat) => push({ kind: 'publish', cat, subCat })}
            />
          )}
          {top.kind === 'publish' && (
            <PublishPage
              showToast={showToast}
              initialCat={top.cat}
              initialSubCat={top.subCat}
              onDone={pop}
              onPublished={() => setStack([])}
              onPay={(amount, label) => push({ kind: 'payment', amount, label, onDone: () => setStack([]) })}
            />
          )}
          {top.kind === 'payment' && (
            <PaymentPage
              ...
              label={top.label}
              onBack={pop}
              onSuccess={top.onDone ?? (() => setStack((s) => s.slice(0, -2)))}
              showToast={showToast}
              allowPoints={top.allowPoints}
            />
          )}
          {top.kind === 'sub' && (
            <ProfileSubPage
