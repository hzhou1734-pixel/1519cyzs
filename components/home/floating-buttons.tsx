...
export function FloatingButtons({ showBackToTop, onFollow, onCustomerService, onCall, onBackToTop }: Props) {
  return (
    <div className="pointer-events-none absolute bottom-0 left-0 z-30 w-full">
      ...
      <div className="pointer-events-auto absolute bottom-24 left-3 flex flex-col items-center gap-2.5">
        <button
          type="button"
          onClick={onFollow}
          className="flex h-11 w-11 items-center justify-center rounded-full bg-accent text-xs font-bold text-accent-foreground shadow-lg shadow-accent/30 transition-transform hover:scale-105 active:scale-95"
          title="关注"
        >
          关注
        </button>
