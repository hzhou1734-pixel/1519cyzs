...
export function PaymentPage({ amount, label, onBack, onSuccess, showToast, allowPoints = false }: Props) {
  const [method, setMethod] = useState('balance')
  const [usePoints, setUsePoints] = useState(false)

  const balance = profile.stats.balance
  const totalPoints = profile.stats.points

  // 本次最多可抵扣金额（受订单金额与账户积分双重限制）
  const maxDeduct = allowPoints ? Math.min(amount, Math.floor(totalPoints / POINTS_PER_YUAN)) : 0
  const deduct = usePoints ? maxDeduct : 0
  const pointsUsed = Math.round(deduct * POINTS_PER_YUAN)
  const payable = Math.max(0, amount - deduct)

  const amountText = `¥${amount.toFixed(2)}`
  const payableText = `¥${payable.toFixed(2)}`
  ...
  return (
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="flex flex-col items-center bg-primary px-4 py-7 text-primary-foreground">
        ...
        <span className="mt-1 text-4xl font-bold tracking-tight">{payableText}</span>
        {deduct > 0 ? (
          <span className="mt-1.5 text-[13px] text-primary-foreground/80">
            原价 <span className="line-through">{amountText}</span> · 已抵扣 ¥{deduct.toFixed(2)}
          </span>
        ) : (
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-4">
        ...
        {allowPoints && (
        <>
        <p className="mb-2 px-1 text-[13px] font-medium text-muted-foreground">积分抵扣</p>
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-border bg-card px-3.5 py-3.5">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-muted text-[#e6a23c]">
            <Coins className="h-5 w-5" />
          </span>
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-bold text-foreground">使用积分抵扣</span>
            <span className="block text-[12px] text-muted-foreground">
              {maxDeduct > 0 ? (
                <>
                  可用 <span className="font-semibold text-foreground">{totalPoints}</span> 积分，本次抵扣{' '}
                  <span className="font-semibold text-accent">{pointsUsed}</span> 积分（¥{deduct.toFixed(2)}）
                </>
              ) : (
                <>
                  当前 <span className="font-semibold text-foreground">{totalPoints}</span> 积分，暂不可抵扣
                </>
              )}
            </span>
          </span>
          <button
            ...
            className={`relative h-6 w-11 shrink-0 rounded-full transition-colors disabled:opacity-40 ${
              usePoints ? 'bg-accent' : 'bg-muted-foreground/30'
            }`}
          >
            <span
        ...
        <p className="mb-2 px-1 text-[13px] font-medium text-muted-foreground">
          {payable > 0 ? '选择支付方式' : '积分已抵扣全部金额，无需其他支付方式'}
        </p>
        <div className={`flex flex-col gap-2.5 ${payable > 0 ? '' : 'pointer-events-none opacity-40'}`}>
          {METHODS.map((m) => {
            const Icon = m.icon
      ...
      <div className="border-t border-border bg-card px-3 pb-6 pt-3">
        ...
        >
          {payable > 0 ? `确认支付 ${payableText}` : '确认使用积分'}
        </button>
      </div>
    </div>
  )
